...
function slugify(value: string) {
  return (
    value.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'page'
  );
}
    ...
    <div className="field">
      ...
      <div className="image-field-row">
        <input
          ...
          placeholder={isData ? '[local file]' : 'https://...'}
          readOnly={isData}
        />
      ...
      {value && (
        <div className="image-thumb-wrap">
          ...
          <div className="image-thumb-info">
            <span>{isData ? 'Local file (stored as base64)' : value}</span>
          </div>
...
function StyleEditor({
  ...
  block: SiteBlock;
  updateBlock: (id: string, patch: Partial<SiteBlock>) => void;
  t: (key: MessageKey) => string;
}) {
  const s: BlockStyle = block.style ?? {};

  function patch(p: Partial<BlockStyle>) {
    updateBlock(block.id, { style: { ...s, ...p } } as Partial<SiteBlock>);
  }
  ...
  return (
    <>
      <div className="editor-header">
        <h2>{t('tab.style')}</h2>
        <p>Customize colors, background and spacing for this block.</p>
      </div>
      <ColorField label={t('field.bgColor')} value={s.bgColor ?? ''} onChange={(bgColor) => patch({ bgColor })} />
      <ImageField
        label={t('field.bgImage')}
        value={s.bgImage ?? ''}
        onChange={(bgImage) => patch({ bgImage })}
      />
      ...
      <div className="field">
        ...
        <select
          value={s.paddingY ?? 'normal'}
          onChange={(e) => patch({ paddingY: e.target.value as PaddingSize })}
        >
...
function AnimationEditor({
  ...
  function patch(p: Partial<BlockAnimation>) {
    updateBlock(block.id, { animation: { ...anim, ...p } } as Partial<SiteBlock>);
  }
  ...
  return (
    <>
      <div className="editor-header">
        <h2>{t('tab.animation')}</h2>
        <p>Animate this block when it enters the viewport on scroll.</p>
      </div>
      <div className="field">
...
function BlockContentEditor({
    ...
    return (
      <>
        ...
        <button
          ...
          onClick={() => updateBlock(block.id, { images: [...block.images, { src: '', caption: 'Image caption' }] })}
        >
          {t('action.addImage')}
        </button>
      </>
        ...
        <button
          ...
          onClick={() =>
            updateBlock(block.id, {
              items: [...block.items, { title: 'Project', category: 'Category', image: '', text: 'Short project story.' }],
            })
          }
        >
          {t('action.addProject')}
    ...
    return (
      <>
          ...
          <div className="repeat-row form-field-row" key={index}>
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={(e) => {
                  const fields = [...block.fields];
                  fields[index] = { ...field, kind: e.target.value as typeof field.kind };
                  updateBlock(block.id, { fields });
                }}
              >
        ...
        <button
          className="soft-button"
          onClick={() =>
            updateBlock(block.id, { fields: [...block.fields, { label: 'New field', kind: 'text', required: false }] })
          }
        >
          {t('action.addField')}
        </button>
      ...
      <>
        ...
        <div className="field">
          ...
          <select
            ...
            onChange={(e) =>
              updateBlock(block.id, { position: e.target.value as 'left' | 'right' })
            }
          >
        ...
        {block.widgets.map((widget, index) => (
          <div className="repeat-row" key={widget.id}>
            <Field
              label={t('field.widgetHeading')}
              value={widget.heading}
              onChange={(heading) => {
                const widgets = [...block.widgets];
                widgets[index] = { ...widget, heading };
                updateBlock(block.id, { widgets });
              }}
            />
            <div className="field">
              ...
              <select
                ...
                onChange={(e) => {
                  const widgets = [...block.widgets];
                  widgets[index] = { ...widget, kind: e.target.value as SidebarWidget['kind'] };
                  updateBlock(block.id, { widgets });
                }}
              >
            ...
            <Field
              label={widget.kind === 'links' ? `${t('field.widgetBody')} (Label | URL per line)` : t('field.widgetBody')}
              value={widget.body}
              textarea
              onChange={(body) => {
                const widgets = [...block.widgets];
                widgets[index] = { ...widget, body };
                updateBlock(block.id, { widgets });
              }}
            />
          </div>
        ...
        <button
          ...
          onClick={() => {
            ...
            updateBlock(block.id, { widgets: [...block.widgets, newWidget] });
          }}
        >
          {t('action.addWidget')}
        </button>
  ...
  const itemLabels =
    block.type === 'services'
      ? [t('field.title'), t('field.text')]
      : block.type === 'testimonials'
        ? [t('field.name'), t('field.text')]
        : [t('field.question'), t('field.answer')];
    ...
    <>
      ...
      {block.items.map((item, index) => (
        <div className="repeat-row" key={index}>
          {Object.entries(item).map(([key, value], fieldIndex) => (
            <Field
              key={key}
              label={itemLabels[fieldIndex]}
              value={value}
              textarea={fieldIndex === 1}
              onChange={(nextValue) => {
                const items = [...block.items] as Array<Record<string, string>>;
                items[index] = { ...items[index], [key]: nextValue };
                updateBlock(block.id, { items } as Partial<SiteBlock>);
              }}
            />
          ))}
        </div>
      ...
      <button
        ...
        onClick={() => {
          const empty =
            block.type === 'services'
              ? { title: 'Service', text: 'Short description.' }
              : block.type === 'testimonials'
                ? { name: 'Client', text: 'A short review.' }
                : { q: 'Question?', a: 'Answer.' };
          updateBlock(block.id, { items: [...block.items, empty] } as Partial<SiteBlock>);
        }}
      >
        {t('action.addItem')}
      </button>
...
export default function App() {
  ...
  function updateActivePageBlocks(blocks: SiteBlock[]) {
    updateActivePage({ blocks });
  }
  ...
  function addPage() {
    ...
    const page: PageContent = {
      ...
      title,
      slug: slugify(title),
      blocks: [createBlock('hero'), createBlock('contact')],
    };
  ...
  return (
    <div className={previewCollapsed ? 'app-shell preview-collapsed' : 'app-shell'}>
      ...
      <aside className="sidebar">
        <div className="brand">
          <img className="brand-logo" src="/weblogo.jpg" alt="Astro Site Builder" />
      ...
      <main className="workspace">
        <section className="topbar">
          <div>
            ...
            <p>
              {activeMenu === 'content'
                ? t('blocks.saved', { count: currentBlocks.length })
                : t(`menuHelp.${activeMenu}` as MessageKey)}
            </p>
          </div>
            ...
            <div className="block-list">
              ...
              <div className="block-stack">
                {currentBlocks.map((block, index) => (
                  <article
                    className={selectedId === block.id ? 'block-card active' : 'block-card'}
                    draggable
                    key={block.id}
                    onDragStart={() => setDraggedId(block.id)}
                    onDragEnd={() => setDraggedId(null)}
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={() => { if (draggedId) reorderBlocks(draggedId, block.id); }}
                  >
            ...
            <div className="panel">
              ...
              <div className="theme-list">
                {Object.entries(themes).map(([id, theme]) => (
                  <button
                    className={site.theme === id ? 'theme-choice active' : 'theme-choice'}
                    key={id}
                    onClick={() => updateSite({ ...site, theme: id as SiteContent['theme'] })}
                  >
                    <span>
